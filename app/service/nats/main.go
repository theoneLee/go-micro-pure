...
func demo() {
	...
	sc.Publish("foo1", []byte("Hello World")) // does not return until an ack has been received from NATS Streaming

	//// Simple Async Subscriber todo 这里会没有接受到消息然后打印
	//sub, _ := sc.Subscribe("foo", func(m *stan.Msg) {
	//	fmt.Printf("Received a message: %s\n", string(m.Data))
	//})
	...
	//// Replying  todo 像官方这样的用法（即先pub，再sub，会有问题，这里会超时）
	//c.Subscribe("help", func(subj, reply string, msg string) {
	//	c.Publish(reply, "I can help!")
	//})
